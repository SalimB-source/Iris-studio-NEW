...
function ContactSpreeForm() {
  const [state, handleSubmit] = useForm("mkjnvbvp");
  if (state.succeeded) return <div className="contact-form-success"><p>Merci — votre message a été envoyé.</p></div>;
  return (
    <form onSubmit={handleSubmit} className="contact-form" aria-label="Formulaire de contact">
      <label>
        <span>Nom</span>
      ...
      <div style={{ marginTop: '0.6rem' }}>
        <button type="submit" disabled={state.submitting} className="button button-orange">Envoyer</button>
      </div>
    </form>
  );
}
